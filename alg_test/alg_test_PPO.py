...
class PPO_quad():
    def __init__(
        ...
        if self.mlp:
            ...
            self.mlp = MLP(observation_space,action_space).to(self.device)
        print('MLP is ready!')
    ...
    def get_actor_critic_action_and_values(self,obs,eval=True):
        ...
        probs = TanhNormal(loc = logits, scale=self.zeta*nn.Sigmoid()(var)+0.3,max=self.minmax[-1],min=self.minmax[0])
        if eval is True:
            action = probs.sample()
            # print(action,logits,var,self.zeta*var+.1)
            print(probs.log_prob(action))
            return action.view(old_shape), probs.log_prob(action).prod(), values
        else:
            action = eval
            dummy_action = probs.sample((500,))
            print(probs.log_prob(action.view(logits.shape)).shape)
    ...
    def train(self):
        ...
        for epoch in range(self.epochs):
            ...
            for iteration, data in enumerate(dataloader):
                ...
                norm_quality                                = (quality-self.qua_var_mean[1])/self.qua_var_mean[0]**.5
...
class MLP(nn.Module):
    ...
    # nn setup
        ...
        torch.nn.init.orthogonal_(lin1.weight,gain=2**.5)
        lin2 = nn.Linear(500,100)
        torch.nn.init.orthogonal_(lin2.weight,gain=2**.5)
        lin3 = nn.Linear(100,action_space)
        torch.nn.init.orthogonal_(lin3.weight,gain=2**.5)
        lin4 = nn.Linear(100,action_space)
        torch.nn.init.orthogonal_(lin4.weight,gain=2**.5)
        lin5 = nn.Linear(100,1)
        torch.nn.init.orthogonal_(lin5.weight,gain=2**.5)
...
class custom_dataset(Dataset):
    ...
    def __init__(self,data,data_size,num_robot,gamma,verbose = False):
        ...
        if verbose:
            ...
            print(len(self.logprob))
            print(len(self.reward))
            print(len(self.timestep))
        ...
        self.local_timestep = torch.vstack(self.timestep).view(-1,1)
        # print(torch.hstack([self.local_reward,self.local_return,self.local_timestep]))
        if verbose:
