...
class Quadrup_env(gym.Env):
    ...
    def get_joints_values(self,client):
        ...
        for Id in self.jointId_list:
            temp_obs_value += [p.getJointState(self.robotId,Id, physicsClientId=client)[0]]
    ...
    def calculate_target(self,client):
        ...
        temp_obs_value += [*(target_dir[:2]/np.linalg.norm(target_dir[:2]))]
        return temp_obs_value
...
# env = Quadrup_env(render_mode = 'human',buffer_length=5,ray_test=False,noise =0,terrain_type=3,terrainHeight=[0,0.05],seed=2,max_length=2000,)
