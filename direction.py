...
while True:
    p.stepSimulation()
    p.addUserDebugLine([0,0,1],np.array([0,0,1])+np.array([0,0,0.5]),lineWidth = 5, lifeTime =.2, lineColorRGB = [0,0,1])
    p.addUserDebugLine([0,0,1],np.array([0,0,1])+np.array([0,0.5,0]),lineWidth = 5, lifeTime =.2, lineColorRGB = [0,1,0])
    p.addUserDebugLine([0,0,1],np.array([0,0,1])+np.array([0.5,0,0]),lineWidth = 5, lifeTime =.2, lineColorRGB = [1,0,0])
    ...
    base_vel =  np.array(list(p.getBaseVelocity(robotId)[0])+[1])
    base_new_vel = utils.bullet_active_rotation(base_inf,(base_inf[0],base_vel))
    p.addUserDebugLine([0,0,1],np.array([0,0,1])+np.array(base_new_vel[1][:3]),lineWidth = 5, lifeTime =.2, lineColorRGB = [0,0,0])
    
    invert_base = p.invertTransform(*base_inf)
    # base_lin_ve, base_ang_ve = p.getBaseVelocity(robotId)
    # base_ang_ve = p.getQuaternionFromEuler(base_ang_ve)
    base_new_vel_new = p.multiplyTransforms(*invert_base,base_vel[:3],[0,0,0,1])
    p.addUserDebugLine(base_inf[0],np.array(base_inf[0])+np.array(base_new_vel_new[0]),lineWidth = 5, lifeTime =.2, lineColorRGB = [1,1,1])
    

    headwbody = ([0.2,0,0],[0,0,0,1])
    headwworld = p.multiplyTransforms(*base_inf,*headwbody)
    p.addUserDebugLine(base_inf[0],headwworld[0],lineWidth = 10, lifeTime =.2, lineColorRGB = [0,0,0])
