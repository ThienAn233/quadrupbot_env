...
class Quadrup_env(gym.Env):
    ...
    def sample_terrain(self, client):
        ...
        # TODO do this better..
        if self.terrain_type == 'maze':
            p.loadURDF('plane.urdf')
            terrain_shape = p.createCollisionShape(
                shapeType = p.GEOM_HEIGHTFIELD,
                meshScale=[.12, .12, .75 if self.terrain_type == "maze" else 1],
            ...
            if self.terrain_type == "maze":
                ...
                p.resetBasePositionAndOrientation(terrain, [0, 0.1, 0], [0, 0, 0, 1])
            else:
                p.resetBasePositionAndOrientation(terrain, [0, 0.1, 0], [0, 0, 0, 1])
        p.configureDebugVisualizer(p.COV_ENABLE_RENDERING,1)

        # TODO do this better..
        if self.terrain_type == 'mount':
            terrain_shape = p.createCollisionShape(
                shapeType = p.GEOM_HEIGHTFIELD,
                meshScale=[.05, .05, 5 if self.terrain_type == "mount" else 1],
                fileName= "heightmaps/wm_height_out.png")
            terrain = p.createMultiBody(0, terrain_shape)
            if self.terrain_type == "mount":
                textureId = p.loadTexture("heightmaps/gimp_overlay_out.png")
                p.changeVisualShape(terrain, -1, textureUniqueId=textureId)
                p.resetBasePositionAndOrientation(terrain, [0, 0, .3], [0, 0, 0, 1])
            else:
                p.resetBasePositionAndOrientation(terrain, [0, 0, .3], [0, 0, 0, 1])
        p.configureDebugVisualizer(p.COV_ENABLE_RENDERING,1)
...
# # # TEST CODE # # #
# env = Quadrup_env(render_mode = 'human',max_length=500,buffer_length=5,terrain_type='maze',seed=1,terrainHeight=[0,0.05],ray_test=False,debug=False)
# obs, info = env.reset()
...
# env = Quadrup_env(render_mode = 'human',buffer_length=5,ray_test=False,noise =0,terrain_type=2,terrainHeight=[0,0.04],seed=2,max_length=2000,)
# # model = SAC.load('SAC_real_gym_2024-05-03-15-57-10.zip',device='cpu',print_system_info=True)
# model = SAC.load('SAC_real_gym_2024-05-12-06-39-22.zip',device='cpu',print_system_info=True)
