...
class Quadrup_env(gym.Env):
    ...
    def __init__(
        ...
        self.radius             = 0.5
        self.initialVel         = [0, 0]
        self.initialMass        = [0, 0]
        self.initialPos         = [0, 0]
        self.initialFriction    = [2, 2]
    ...
    def sample_target(self,client):
        ...
        # Sample new velocity
        init_vel    = 0 #np.random.normal(loc = self.initialVel[0],scale = self.initialVel[1],size=(3))
        ...
        new_friction = 1.5 #np.random.uniform(*self.initialFriction)
    ...
    def sample_terrain(self, client):
        ...
        if terrain_type == 0 :
            ...
            self.zz_height[0] = 0
        if terrain_type == 1 :
            a, b, c = np.random.uniform(0.15,0.15), np.random.uniform(.8,.8), np.random.uniform(.8,.8)
        ...
        if terrain_type == 2 :
            a, b, c =  np.random.uniform(0.2,0.2), np.random.uniform(.8,.8), 10*self.terrainHeight[-1]
            zz = np.round(a*(np.sin(b*xx-np.pi/2)),1)*c
            self.zz_height[0] = -a*c
        if terrain_type == 3 :
            a = np.random.uniform(2.,2.)        # cang lon thi dinh cang lon (so luong bac thang)
            b = np.random.uniform(1.5,1.5)        # cang lon thi ban kinh cang nho (ban kinh vong thang)
...
# # # TEST CODE # # #
# env = Quadrup_env(render_mode = 'human',max_length=500,buffer_length=5,terrain_type=3,seed=1,terrainHeight=[0,0.05],ray_test=True,debug=True)
# obs, info = env.reset()
...
# env = Quadrup_env(render_mode = 'human',buffer_length=5,ray_test=False,noise =0,terrain_type=1,terrainHeight=[0,0.05],seed=2,max_length=2000,)
