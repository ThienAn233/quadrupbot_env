...
class Quadrup_env():
    def __init__(
        ...
        x_n                 = np.linspace(-0.5,0.5,self.num_ray)
        y_n                 = np.linspace(-0.5,0.5,self.num_ray) 
        ...
        if self.debug:
            for client in self.clientId:
                for i in range(self.num_ray**2):
                    self.rayId_list += [p.addUserDebugLine([0,0,0], [0,0,1], [0,0,0], physicsClientId=client)]
                for i in range(7):
    ...
    def sample_terrain(self, client):
        ...
        y = np.linspace(-self.terrain_shape[1]/2,self.terrain_shape[1]/2,int(numHeightfieldColumns))
        ...
        if terrain_type == 3 :
            ...
            zz = c*np.round(a*(np.sin(b*xx-np.pi/2)+np.sin(b*yy-np.pi/2)))
            self.zz_height[client] = c*a*2
    ...
    def get_ray_test(self,client):
        ...
        for info in p.rayTestBatch(start,end,numThreads=0,physicsClientId =client):
            ray_contact += [info[3]]
            ray_tio     += [info[2]]
        self.ray_start_end[client] = [start,end,np.array(ray_contact),np.array(ray_tio)]
    ...
    def viz_ray(self,client):
        base_pos    = self.base_pos[client]
        start, end, contact, frac  = self.ray_start_end[client]
        for i in range(self.num_ray**2):
            p.addUserDebugLine(start[i],contact[i], lineColorRGB = [0,0,0],replaceItemUniqueId=self.rayId_list[i],physicsClientId = client)
