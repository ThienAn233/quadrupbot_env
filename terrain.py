...
if heightfieldSource==useProgrammatic:
  numHeightfieldRows = 600
  numHeightfieldColumns =600
  x = np.linspace(-6,6,numHeightfieldRows)
  y = np.linspace(-6,6,numHeightfieldColumns)
  xx, yy =np.meshgrid(x,y)
  a = 3.        # cang lon thi dinh cang lon (so luong bac thang)
  b = 2.        # cang lon thi ban kinh cang nho (ban kinh vong thang)
  c = 0.15      # cao do bac thang
  noise = 0.05*np.random.random(xx.shape)
  zz = c*np.round(a*(np.sin(b*xx-np.pi/2)+np.sin(b*yy-np.pi/2))) + noise 
  ...
  print("terrain ID: ",terrain)
  p.resetBasePositionAndOrientation(terrain,[0,0,2*a*c], [0,0,0,1])
...
if heightfieldSource==useTerrainFromPNG:
  terrainShape = p.createCollisionShape(shapeType = p.GEOM_HEIGHTFIELD, meshScale=[0.01,0.01,2.4],fileName = "heightmaps/wm_height_out.png")
  terrain  = p.createMultiBody(0, terrainShape)
